...
//   // מופע AngularFire ראשי (אם צריך למקומות אחרים באפליקציה)
//   constructor(private primaryAuth: AngularFireAuth) {}

//   /** ===== מופע Firebase משני כדי לא להחליף את סשן המזכירה ===== */
...
//   /** סיסמה זמנית בת ~8 תווים, בטוחה לשימוש בדפדפן */
...
//   /**
//    * יוצר משתמש חדש בפיירבייס אם המייל לא רשום – דרך המופע המשני (לא פוגע בסשן של המזכירה).
...
//     email = email.trim().toLowerCase(); // ← נרמול

//     // בדיקה אם כבר רשום
//     const methods = await fetchSignInMethodsForEmail(this.secondaryAuth, email);
//     if (methods?.length) {
//       const err = new Error('משתמש עם המייל הזה כבר קיים.');
//       (err as any).code = 'auth/email-already-in-use'; // ← קוד רשמי
//       throw err;
//     }
...
//     await fbSignOut(this.secondaryAuth); // לא שובר סשן של המזכירה
...
//     } else if (code === 'auth/email-already-in-use') { // ← עברנו לקוד הרשמי
//       this.errorMessage = 'משתמש עם המייל הזה כבר קיים.';
//     } else if (code === 'auth/operation-not-allowed') {
//       this.errorMessage = 'הרשמת אימייל/סיסמה אינה פעילה בפרויקט Firebase.';
//     } else if (code === 'auth/weak-password') {
//       this.errorMessage = 'הסיסמה חלשה מדי. נסי שוב.';

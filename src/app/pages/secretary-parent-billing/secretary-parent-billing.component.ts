...
import { CollectPaymentsDialogComponent } from './collect-payments-dialog.component';
...
canGoNext = computed(() => this.pageIndex() + 1 < this.totalPages());
  ...
  chargeStatusLabel(charge: ChargeWithPaymentStatus): string {
    if (charge.status === 'failed' || this.isParentPaymentFailed(charge.parent_uid)) {
      const reason = charge.failure_reason
        ? paymentFailureReason({ message: charge.failure_reason })
        : null;
      const shortReason = reason?.split('.')[0]?.trim();
      return shortReason ? `נכשל — ${shortReason}` : 'נכשל';
    }
...
// סיבת הכישלון נשמרת בטבלת charges ומצורפת גם אם ה-view עדיין לא חושף אותה.
const loadedChargeIds = (rows ?? []).map((c: any) => c.id).filter(Boolean);
const failureReasonByChargeId = new Map<string, string | null>();

if (loadedChargeIds.length) {
  const { data: failureRows, error: failureRowsError } = await dbTenant()
    .from('charges')
    .select('id,failure_reason')
    .in('id', loadedChargeIds);

  if (failureRowsError) throw failureRowsError;

  for (const row of failureRows ?? []) {
    failureReasonByChargeId.set(row.id, row.failure_reason ?? null);
  }
}
...
this.chargesSummary.set(summary);
    ...
    const rowsWithPaymentStatus = (rows ?? []).map((c: any) => {
      ...
      return {
        ...c,
        failure_reason: c.failure_reason ?? failureReasonByChargeId.get(c.id) ?? null,
        hasPaymentMethod,
...
`).eq('related_charge_id', chargeId)
  ...
  async openChargeSelectedDialog() {
    const selected = this.openCharges()
      ...
      .map(c => ({
        ...
        parentUid: c.parent_uid,
        parentName: c.parent_name || `${c.first_name || ''} ${c.last_name || ''}`.trim(),
        amountAgorot: this.realRemainingAgorot(c),
        hasPaymentMethod: c.hasPaymentMethod,
        hasExpiredPaymentMethod: c.hasExpiredPaymentMethod,
      }));
  ...
  async collectSelectedPayments(rows: any[]) {
    ...
    try {
      ...
      const succeededCount = reportRows.filter(x => x.status === 'success').length;
      const failedCount = reportRows.filter(x => x.status === 'failed').length;

      // הבחירות נשארות מסומנות בזמן הצגת הדוח ומתאפסות רק לאחר אישורו.
      ...
      reportRef.afterClosed().subscribe(() => {
        ...
        this.successMessage.set(
          failedCount === 0
            ? `כל ${succeededCount} החיובים בוצעו בהצלחה`
            : `ההרצה הסתיימה: ${succeededCount} עברו, ${failedCount} נכשלו`
        );
      });
    } catch (e: any) {

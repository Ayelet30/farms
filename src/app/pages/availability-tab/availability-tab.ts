...
export class AvailabilityTabComponent implements OnInit {
    ...
    try {
      ...
      for (const r of changedRanges) {
        ...
        if (impact && impact.lessons.length > 0) {
          ...
          if (this.mode !== 'secretary') {
            this.toast('קיימים שיעורים שיושפעו מהשינוי. יש לפנות למזכירה לצורך טיפול.');
          }
  ...
  async approveUpdate() {
    if (this.mode !== 'secretary') {
      this.toast('רק מזכירה יכולה לאשר שינוי שמשפיע על שיעורים קיימים');
      return;
    }
  ...
  canHandleImpactedLessons(): boolean {

  ...
  async (req, res) => {
    try {
      ...
      const { data: reqRow, error: reqErr } = await sbTenant
        ...
        .select('id, status, child_id, instructor_id, from_date, payload')
        .eq('id', requestId)
        .maybeSingle();
      if (reqErr) throw reqErr;
      if (!reqRow) throw new Error('Request not found');

      if (String((reqRow as any).status) !== 'APPROVED') {
        throw new Error('Request is not APPROVED (won’t send email)');
      }
      ...
      try {
        ...
        mailOk = false;
        warning = 'השיעור אושר, אך שליחת המייל להורה נכשלה';

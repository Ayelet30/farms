// functions/src/reject-add-child-and-notify.ts
...
export const rejectAddChildAndNotify = onRequest(
  {
    region: 'us-central1',
  ...
  async (req, res) => {
    try {
      ...
      const newStatus = system ? 'REJECTED_BY_SYSTEM' : 'REJECTED';

      // 2) update request rejected (רק אם PENDING)
      ...
      try {
        ...
        console.warn('rejectAddChildAndNotify: email failed but reject OK', err);
      }
      ...
      console.error('rejectAddChildAndNotify error', e);
